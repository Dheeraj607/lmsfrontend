...
export default function RegisterForm() {
  ...
  useEffect(() => {
    ...
    if (pkg) {
      ...
      console.log("Selected Package Data:", parsed);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // store user details for OTP
      localStorage.setItem("registerName", formData.name);
      localStorage.setItem("registerEmail", formData.email);
      localStorage.setItem("registerPhone", formData.phone);

      // store selected package info for OTP page
      if (selectedPackage) {
        localStorage.setItem("selectedPackageName", selectedPackage.name);
        localStorage.setItem("selectedPackageRate", selectedPackage.finalPrice.toString());
      }
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-4 max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
        ...
        {selectedPackage && (
          <div className="p-3 bg-gray-100 rounded text-sm mb-2">
            <strong>Selected Package:</strong> {selectedPackage.name} <br />
            Rate: {selectedPackage.finalPrice} INR
          </div>
        )} */}
